    ...
    return (
        <div id='navbar' className={`fixed pb-5 md:pb-5 lg:pb-6 lg:pt-4 flex my-0 items-center justify-between w-screen z-50 ${isDarkMode ? 'my-bg-dark' : 'my-bg'}`} >
            ...
            <div className={`${!isOpen && 'hidden'} md:hidden sm:text-lg fixed flex flex-col items-center top-24 right-0 w-screen text-center pt-2 font-bold`}>
                ...
                <i className={`my-bg opacity-75 w-40 my-2 rounded-full shadow-lg px-8 py-6 fa-solid fa-${isDarkMode ? 'sun my-bg-dark' : 'moon'} fa-xl hover:cursor-pointer ${isDarkMode ? 'text-slate-300' : 'text-purple-700'}`} onClick={onSwitchMode} onClick={closeMenu}></i>
            </div>
